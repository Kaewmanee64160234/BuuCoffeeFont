              ...
              <v-stepper-window-item :value="1">
                <v-form ref="form" v-model="valid">
                  <v-row>
                    ...
                    <v-col cols="12" sm="6">
                      <v-select v-model="productStore.product.category.categoryName" :items="categoryStore.categoriesForCreate.map(category => category.categoryName)" label="เลือกหมวดหมู่" dense :rules="categoryRules" @change="checkCategory" />
                    </v-col>
                  </v-row>
                </v-form>
              </v-stepper-window-item>
                ...
                <v-form ref="form" v-model="valid">
                  <v-container>
                    <v-row>
                      <v-col cols="12">
                        <v-subheader>{{ step.label }}</v-subheader>
                        <v-text-field variant="solo" v-model="getProductType(step.label).productTypePrice" label="ราคาประเภทสินค้า" type="number" required :rules="productTypePriceRules" />
                      </v-col>
                      <v-col cols="12">
                        <v-btn icon @click="() => addRecipe(getProductType(step.label))">
                          <v-icon>mdi-plus</v-icon>
