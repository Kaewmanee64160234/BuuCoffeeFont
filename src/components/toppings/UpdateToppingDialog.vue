<template>
  ...
  watch(() => toppingStore.selectedTopping, (newVal) => {
    if (newVal) {
      toppingName.value = newVal.toppingName;
      toppingPrice.value = newVal.toppingPrice;
    }
  }, { immediate: true });
  ...
  const submitForm = async () => {
    ...
    const updatedTopping: Topping = {
      toppingId: toppingStore.selectedTopping?.toppingId || 0,
      toppingName: toppingName.value,
      toppingPrice: toppingPrice.value,
    };
  
    try {
      await toppingStore.updateTopping(updatedTopping);
