...
const submitForm = async () => {
  if (form.value && form.value.validate()) {
    if (cashierAmount.value !== null) {
      ...
      try {
        console.log("ปิดการขาย cashier", cashier);
        console.log("ปิดการขาย items", items);
...
<template>
  <v-dialog v-model="ReportFinnceStore.closeCashierDialog" max-width="500px">
    <v-card>
      <v-card-title class="text-h5 bg-brown-lighten-4 pa-4 d-flex align-center justify-space-between">
        <div class="d-flex align-center">
          <v-icon class="mr-2">mdi-cash</v-icon>
          บันทึกยอดเงินสดก่อนปิดการขาย
        </div>
        <v-btn icon @click="ReportFinnceStore.closeCashierDialog = false">
