...
<template>
  ...
  <div class="h-screen app">
    ...
    <v-window v-model="step" transition="fade" class="h-screen window-container">
      <!-- Order Details View -->
          ...
          <div class="header">
            ...
            <div>
              <v-row>
                ...
                <v-col cols="12" md="6" class="text-end">
                  <p @click="cancelReceipt()" class="cancel-receipt">ยกเลิกรายการ</p>
                </v-col>
              </v-row>
            </div>
            ...
            <div class="selected-items-list-50">
              <v-list class="full-width" style="height: 25vh;">
                <v-list-item-group>
                  <div v-for="(item, index) in selectedItems" :key="index" class="selected-item full-width my-2 card-item">
                    <v-list-item @click="openProductCardDialog(item)" :prepend-avatar="`${url}/products/${item.product?.productId}/image`" class="full-width item-row">
                      <v-row no-gutters>
            ...
            <div class="summary-section-30">
              ...
              <div class="promotion-30">
                <div class="sub-promotion">
                  <div v-for="(promotion) in posStore.receipt.receiptPromotions" :key="promotion.receiptPromotionId" class="promotion-row">
                    <span class="promotion-name">{{ promotion.promotion.promotionName }}:</span>
                    <span class="promotion-discount red--text">{{ promotion.discount }} $</span>
          ...
          <div class="payment-method">
            ...
            <v-row class="pa-3 justify-center">
              <div class="payment-buttons-container mt-4">
                <v-btn :class="{ 'selected': posStore.receipt.paymentMethod === 'cash' }" class="payment-button" @click="selectPaymentMethod('cash')">
                  เงินสด
                </v-btn>
                <v-btn :class="{ 'selected': posStore.receipt.paymentMethod === 'qrcode' }" class="payment-button" @click="selectPaymentMethod('qrcode')">
                  แสกนจ่าย
                </v-btn>
              </div>
            </v-row>
            ...
            <div class="ma-2">
              ...
              <div v-if="posStore.receipt.paymentMethod === 'cash'">
                <p class="d-flex justify-space-between align-center pr-6 my-2">
                  ...
                  <v-text-field v-model="posStore.receipt.receive" type="number" variant="solo" label="จำนวนเงิน" style="width: 50%;"></v-text-field>
                </p>
              </div>
