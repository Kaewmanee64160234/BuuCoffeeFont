  ...
  <v-card class="selected-items-list">
    ...
    <v-list>
      <v-list-item v-for="(item, index) in selectedItems" :key="index">
        ...
        <!-- <v-list-item-content>{{ item.productTypeToppings[0].productType.product?.productName }}</v-list-item-content>
...
const selectedItems = computed(() => posStore.receipt.receiptItems);
