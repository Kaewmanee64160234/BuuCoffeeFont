...
export const useCateringEventStore = defineStore('cateringEvent', () => {
    ...
    const fetchCateringEvents = async () => {
        try {
            const response = await axios.get<HistoryCateringEvent[]>('http://localhost:3000/catering-event');
    ...
    const updateStatus = async (id: string, status: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.patch(`http://localhost:3000/catering-event/${id}/status`, { status });
    ...
    const cancelEvent = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.patch(`http://localhost:3000/checkingredients/${id}/cancel`);
                await fetchCateringEvents(); 
                Swal.fire(
                    'ยกเลิกแล้ว!',
                    'เหตุการณ์นี้ถูกยกเลิก.',
                    'success'
                );
            } catch (error) {
