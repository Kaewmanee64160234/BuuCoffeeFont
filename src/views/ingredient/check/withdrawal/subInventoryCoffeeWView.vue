...
const getAllIngredients = () => {
    all_ingredients.value = ingredientStore.all_ingredients.map(ingredient => {
        const subInventory = subIngredientStore.subingredients_coffee.find(
            (sub) => sub.ingredient.ingredientId === ingredient.ingredientId && sub.quantity > 0
        );

        return {
            ...ingredient,
            disabled: !!subInventory
        };
    });
};
        ...
        <v-card-title>
            <v-row>
                <v-col cols="9" style="padding: 20px;">
                    <h3>นำเข้าวัตถุดิบเข้าร้านกาแฟ</h3>
                </v-col>
            </v-row>
            <v-row>
                <v-col cols="3">
                ...
                <v-container>
                    <v-row>
                        <v-col cols="3" style="text-align: center; padding: 8px"
                            v-for="(item, index) in all_ingredients.values" :key="index">
                            <v-card width="100%" :class="{ 'disabled-card': item.disabled }"
                                :style="{ height: '200px', cursor: item.disabled ? 'not-allowed' : 'pointer' }"
                                @click="!item.disabled && ingredientStore.Addingredienttotable(item)"
