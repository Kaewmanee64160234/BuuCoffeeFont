    ...
    <v-card class="mb-5" style="background-color: #62544a;">
        <v-card-text>
            <v-expansion-panels class="mb-5">
                <v-expansion-panel v-for="(meal, index) in cateringStore.meals" :key="index" class="mb-4">
                    <v-expansion-panel-header>
                        <v-row class="d-flex align-center">
                            <v-col class="d-flex align-center">
                                <v-card-title>รายละเอียดมื้ออาหาร #{{ index + 1 }}</v-card-title>
                                <v-btn icon class="ml-2" @click="cateringStore.removeMeal(index)">
...
<v-row class="mt-4"> <!-- เพิ่มระยะห่างด้านบน -->
    <v-col cols="6">
        <v-container>
            <v-row>
    ...
    <v-col cols="6">
        <v-card style="height: 400px; overflow-y: auto;">
            <v-table>
                ...
                <tbody>
                    <tr v-for="(item, itemIndex) in meal.mealIngredient" :key="itemIndex">
                        ...
                        <td>
                            <button @click="removeIngredientFromMeal(index, itemIndex)" class="styled-button">
                                ลบ
                            </button>
                        </td>
                    </tr>
            ...
            <v-btn style="background-color: #ff9642;" @click="cateringStore.addMeal" class="button-full-width" rounded>
